...
var app = new Vue({
    ...
    computed:{
        ...
        getSelfSecMoney:function(){
            var money = (this.pk.salary * 12 / 17520 * this.nowTime).toFixed(2)
            return this.getFixedMoney(money)
        },
        getOtherSecMoney:function(){
            var money = (11000 * 12 / 17520 * this.nowTime).toFixed(2)
            return this.getFixedMoney(money)
        },
        getStarSecMoney:function(){
            var money = (this.star[this.starActive].salary / 17520 * this.nowTime).toFixed(2)
            return this.getFixedMoney(money)
        },
    },
    mounted:function(){
    ...
    methods:{
        ...
        setScrollDataRank:function(){
            ...
            var config = {
                ...
                index:this.starActive,
                leftCallback:this.showNext,
                rightCallback:this.showPrev,
                length:this.rank[this.activeStatus].content[this.starActive].list.length
            };
            if(this.activeStatus == 3 && this.starActive == 0){
                config.one = 1246;
        ...
        setSecMoney:function(){
            setTimeout(this.setSecMoney,1000)
            this.nowTime++
        },
        ...
        initCanvas:function(){
            ...
            imgs.forEach(function(url){
                ...
                img.onload = function(){
                    self.load++
                    if(self.load == imgs.length){
                        self.ctx.drawImage(loadedImgs[0],0,0)
                        self.ctx.drawImage(loadedImgs[1],487,849)
                        if(self.drawStatus){
                            self.ctx.drawImage(loadedImgs[2],0,0)
                            self.drawContent()
                        }
                    }
                }
        ...
        drawContent:function(){
            ...
            this.ctx.fillText("罗纳尔多年薪5000万",38,46+36)

            var twidth = this.ctx.measureText("罗纳尔多年薪5000万").width
            this.ctx.beginPath()
            this.ctx.lineWidth = 1
            ...
            this.ctx.fillText(this.pk.name+"和罗纳尔多一样，",38,132+26)
